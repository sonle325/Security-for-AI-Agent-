...
- [II. Nội dung và Phương pháp](#ii-nội-dung-và-phương-pháp)
  ...
  - [2.3. Quản lý cấu hình tập trung](#23-quản-lý-cấu-hình-tập-trung)
  - [2.4. Thu thập dữ liệu kép (Dual Telemetry)](#24-thu-thập-dữ-liệu-kép-dual-telemetry)
  - [2.5. Correlation Engine](#25-correlation-engine)
  - [2.6. Detection & Risk Scoring Engine](#26-detection--risk-scoring-engine)
  - [2.7. Response Engine & Containment](#27-response-engine--containment)
  - [2.8. AI Security Analyzer (NLP)](#28-ai-security-analyzer-nlp)
  - [2.9. Incident Graph & Web Dashboard](#29-incident-graph--web-dashboard)
  - [2.10. Logging Framework](#210-logging-framework)
  - [2.11. Cấu trúc thư mục Source Code](#211-cấu-trúc-thư-mục-source-code)
...
| STT | Bảng | Mô tả |
|:---:|---|---|
| 1 | Bảng 1.1 | So sánh EDR truyền thống vs Hệ thống đề xuất |
| 2 | Bảng 2.1 | Danh sách 5 Sysmon Event IDs được giám sát |
| 3 | Bảng 2.2 | Cấu trúc Config Sections trong config.yaml |
| 4 | Bảng 2.3 | Bảng trọng số Prompt Injection Patterns (9 patterns) |
| 5 | Bảng 2.4 | Bảng trọng số Response Monitor Patterns (12 patterns) |
| 6 | Bảng 2.5 | Công thức Risk Scoring (5 biến số) |
| 7 | Bảng 2.6 | Bảng mapping Severity theo Score |
| 8 | Bảng 2.7 | Mapping Containment Result → Report Status |
| 9 | Bảng 3.1 | Kết quả chạy 8 kịch bản Demo |
| 10 | Bảng 3.2 | Kết quả Unit Testing (41 tests / 5 modules) |
...
### 1.4. So sánh với giải pháp hiện có

**Bảng 1.1 — So sánh EDR truyền thống vs Hệ thống đề xuất:**
...
| **Phát hiện Prompt Injection** | ❌ Không có khả năng | ✅ PromptMonitor (9 patterns, weighted scoring) |
...
| **Nhắm tới đối tượng** | Server, Workstation chung | **Endpoint của Developer** (IDE + AI Agent) |
| **Chi phí triển khai** | Enterprise license ($$$) | Open-source, Python, chạy local |

Điểm đổi mới cốt lõi của hệ thống đề xuất nằm ở khả năng quan sát **Intent Space** — tầng ý định của AI Agent mà không giải pháp EDR thương mại nào hiện tại có thể nhìn thấy.
...
### 2.3. Quản lý cấu hình tập trung (Config-driven Architecture)

Toàn bộ thông số hệ thống (thresholds, credentials, whitelist, model name, ports) được quản lý trong **một file duy nhất** `config.yaml`. Không có giá trị nào được hardcode trong source code — từ module cốt lõi (Detection, Correlation) đến module phụ trợ (Download Model, Web Dashboard, Graph Builder) đều đạt chuẩn **100% Config-driven**.

**Config Loader** (`config_loader.py`) cung cấp API thống nhất:
```python
...
| `whitelist_parent_images` | IDE processes (chống False Positive) | `["code.exe", "cursor.exe", ...]` |
| `nlp` | Model name, labels, HF mirror | `cross-encoder/nli-deberta-v3-small` |
| `risk_weights` | Trọng số Risk Scoring | `rule: 20, process: 20, ...` |
| `tool_monitor` | Excessive/Mass enum limits | `10 calls/30s`, `5 files/10s` |

Ý nghĩa thực tiễn: Khi triển khai ở môi trường Enterprise, quản trị viên SOC có thể điều chỉnh mọi ngưỡng phát hiện, danh sách whitelist, thậm chí đổi mô hình NLP chỉ bằng cách sửa một file YAML duy nhất mà không cần chạm vào source code.

### 2.4. Thu thập dữ liệu kép (Dual Telemetry)

#### 2.4.1. AI Runtime Telemetry Layer

AI Telemetry được thu thập qua **IPC Channel** với cơ chế 2 lớp vận chuyển:

- **Lớp 1 — Windows Named Pipe** (`\\.\pipe\ai_edr_telemetry`): Giao tiếp nội bộ hiệu năng cao, không qua TCP/IP stack.
- **Lớp 2 — TCP Socket fallback** (port 9999): Tự động chuyển sang khi Named Pipe không khả dụng (ví dụ: AI Agent chạy trong container hoặc môi trường không phải Windows).

**Giao thức:** JSON over newline, mỗi event là một JSON object kết thúc bằng `\n`.

**Cơ chế bảo mật IPC:**
- **Auth Token:** Mỗi event phải chứa `auth_token` hợp lệ (đọc từ `config.yaml`). Event không có token hoặc token sai sẽ bị reject ngay lập tức — chống Local Fake Event Injection.
- **Queue maxsize:** Tất cả hàng đợi nội bộ đều set giới hạn dung lượng để chống DoS (kẻ tấn công spam hàng triệu fake event nhằm tràn RAM).

```
AI Agent (Cursor/Copilot) → SDK Client → IPC Channel → IPC Server → 3 Monitors → ai_event_queue
```

**SDK Client API** (`ai_telemetry/sdk_client.py`):

SDK cung cấp API đơn giản để bất kỳ AI Agent nào tích hợp và gửi telemetry về EDR:

```python
from ai_telemetry.sdk_client import AITelemetryClient

client = AITelemetryClient(agent_name="Cursor")
client.connect()  # Named Pipe → TCP fallback
...
4 loại event được hỗ trợ: `prompt` · `response` · `tool_invocation` · `agent_action`. Mỗi event tự động được đính kèm `session_id`, `timestamp`, `agent_name` và `auth_token`.
...
### 2.6. Detection & Risk Scoring Engine

#### 2.6.1. Detection Engine
...
#### 2.6.2. Risk Scoring Engine — Công thức 5 biến số
...
### 2.7. Response Engine & Containment
...
### 2.9. Incident Graph & Web Dashboard

#### 2.9.1. Neo4j Graph Database
...
#### 2.9.2. Web Dashboard Độc Lập (3 Tabs)
...
Hệ thống sử dụng Python `logging` module thay thế `print()` rời rạc, cấu hình tập trung trong `main.py` với 2 handler:

| Handler | Level | Đầu ra |
|---|---|---|
| Console (`StreamHandler`) | INFO+ | Terminal (stdout) |
| File (`FileHandler`) | DEBUG+ | `logs/edr_engine.log` (UTF-8) |

Mỗi module sử dụng logger riêng với namespace phân cấp, giúp dễ dàng lọc và điều tra:

```
EDR.Main          # main.py — Điểm khởi động
EDR.IPC           # ipc_server.py — Nhận AI telemetry
EDR.Correlation   # correlation_engine.py — Liên kết sự kiện
EDR.Detection     # rule_engine.py — Phát hiện
EDR.RiskScoring   # risk_scoring.py — Chấm điểm rủi ro
EDR.Containment   # containment.py — Tiêu diệt tiến trình
EDR.NLP           # nlp_classifier.py — Phân tích NLP
EDR.Neo4j         # graph_builder.py — Đồ thị điều tra
EDR.Summary       # incident_summary.py — Tổng hợp báo cáo
EDR.Sysmon        # sysmon_listener.py — Thu thập OS events
EDR.Config        # config_loader.py — Quản lý cấu hình
```

Ưu điểm:
- Dễ dàng lọc log theo module: `grep "EDR.Correlation" logs/edr_engine.log`
- File log phục vụ post-incident forensics (điều tra hậu sự cố)
- Có thể thay đổi log level per-module mà không restart hệ thống

### 2.11. Cấu trúc thư mục Source Code
...
├── config_loader.py                 # Đọc và map config (cache 1 lần)
├── download_model.py                # Script tải AI Model (DeBERTa)
...
│   ├── prompt_monitor.py            # Phát hiện Prompt Injection (9 patterns)
│   ├── tool_monitor.py              # Phát hiện Tool Anomaly (inc. File Access)
...
│   └── correlation_engine.py        # Intent–Action Correlation + Session Tracking
│
├── detector/                        # Tầng 4 & 6: Phát hiện + Ngăn chặn
│   ├── rule_engine.py               # Rule-based + Allowlist (regex word boundary)
│   ├── risk_scoring.py              # Risk Score Engine (5 biến số)
│   └── containment.py               # Kill Process + Truthful Reporting
│
├── analyzer/                        # Tầng 5: Phân tích NLP
│   ├── nlp_classifier.py            # DeBERTa Zero-Shot NLP (8 labels)
│   └── incident_summary.py          # Sinh báo cáo incident trung thực
│
...
│   ├── mock_web_agent.py            # AI Web Agent simulation
...
Tổng cộng: **~30 file source code**, tổ chức theo kiến trúc phân tầng (Layered Architecture) với trách nhiệm rõ ràng cho từng module.
