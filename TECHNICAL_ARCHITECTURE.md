...
- Vẽ đồ thị nodes + edges khi click vào incident
- 7 node types với màu sắc riêng: Incident (đỏ), AIAgent (xanh dương), Process (xanh lá), Endpoint (vàng), PromptEvent (tím), ToolAnomaly (cam), DataLeak (vàng đậm)
- Hiển thị mối quan hệ: TRIGGERED, ON, EXECUTED, PROMPT, TOOL, LEAK
...
#### Tab 3 — Attack Chain (Mới)
- Link incidents cùng `session_id` thành chuỗi step-by-step
- Mỗi step hiển thị: ID, type, severity, agent, action, timestamp
- Click vào step để xem chi tiết
- Hiển thị mũi tên ↓ giữa các steps
...
- **Detail Panel**: Chi tiết severity, agent, action, cmdline, prompt score, tool risk, data risk
...
| Chưa có MITRE ATT&CK mapping | Trung bình | Map T1059, T1055, T1041, T1003... |
| Telemetry plaintext qua IPC | Trung bình | TLS/HMAC cho Named Pipe channel |
| Correlation chỉ dùng tool name | Trung bình | Bổ sung Process Tree (PID→ParentPID) |
| Dashboard SVG tĩnh | Thấp | Force-directed graph + zoom/pan |
...
| Process Tree Correlation | PID→ParentPID chain, không chỉ tool name | P0 |
| MITRE ATT&CK Mapping | Gắn technique ID cho mỗi Incident | P1 |
| Multi-Agent Telemetry | Agent federation, cross-session tracking | P1 |
| Linux eBPF Collector | Thay thế Sysmon trên Linux | P1 |
| LLM Investigation Assistant | Chatbot điều tra sự cố tự động | P2 |
